...
void Subgraph::SetLabel(std::string _name)
{
    m_strLabel = _name;
}
std::string Subgraph::GetLabel()
{
    return m_strLabel;
}
GraphNode* Subgraph::AddNode(const std::string& _label, const std::string& _id)
{
...
Edge* Subgraph::AddEdge(GraphNode* _src, GraphNode* _dest, const std::string& _label)
{
    Edge* edge = new Edge();
...
Subgraph* Subgraph::AddSubgraph(const std::string& _label, const std::string& _id)
{
...
bool Subgraph::WriterReport(std::ostream& _out, std::string _prefix, int _depth)
{
